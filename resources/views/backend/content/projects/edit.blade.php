...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-10 container-p-y container-fluid">
            <form id="form" method="POST" action="{{ route('admin.projects.update', [$projectType->getSlug(), $project->getId()]) }}" class="form-horizontal">
                @csrf
                @method('PATCH')
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0">@lang('Edit Project')</h5>
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-12 col-md-6 mb-4">
                                ...
                                <select name="category" id="category" class="form-select select2 projectCategory"
                                        data-placeholder="{{ __('Category') }}" data-allow-clear="true">
                                    <option></option>
                                    @foreach(\App\Domains\Projects\Enums\ProjectCategoryEnum::cases() as $option)
                                        <option value="{{ $option->value }}"
                                                @if($project->getCategory()?->value == $option->value) selected @endif>{{ $option->value }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <select name="category_status" id="category_status" class="form-select select2"
                                        data-placeholder="{{ __('Project Type') }}" data-allow-clear="true">
                                    <option></option>
                                </select>
                            </div>
                        ...
                        <div class="form-group row mb-3 mt-1">
                            ...
                            <div class="col-md-10">
                                <input type="text" name="name" class="form-control" placeholder="{{ __('Name') }}"
                                       value="{{ $project->getName() }}" maxlength="100" required/>
                            </div>
                        </div>
                        <div class="form-group row mb-3 mt-1">
                            <label for="description" class="col-md-2 col-form-label">@lang('Description')</label>
                            <div class="col-md-10">
                                <textarea id="autosize-demo" name="description" rows="2" class="form-control" placeholder="{{ __('Description') }}">{{ $project->getDescription() }}</textarea>
                            </div>
                        </div>
...
{{--                                    <option value="{{$project->getClientId()}}" selected> {{$project->getClient()?->getCompany()?->getName()}}  </option>--}}
{{--                                </select>--}}
{{--                            </div>--}}
{{--                        </div>--}}
                        ...
                        <div class="form-group row mb-3 mt-1" id="clientElement">
                            ...
                            <div class="col-md-10">
                                <select name="company_id" id="company_id" class="form-control select2 companies-select"
                                        data-placeholder="{{ __('Client Selection') }}">
                                    <option value="{{$project->getCompanyId()}}" selected> {{$project->getCompany()?->getName()}}  </option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group row mb-3 mt-1">
                            ...
                            <div class="col-md-10">
                                <div class="input-group">
                                    <input type="text" class="form-control" name="sales_cost" placeholder="Amount" aria-label="Amount (to the nearest euro)" value="{{ $project->getSalesCost() }}">
                                ...
                                <div class="form-group row mb-1 mt-1">
                                       <div class="col-md-12">
                                           ...
                                           <select name="status" id="filter_status" class="form-control select2" data-placeholder="{{ __('Status Selection') }}">
                                               @if($hasAccessToStatus ?? false)
                                                    @foreach($projectStatus as $status)
                                                        <option value="{{ $status->getId() }}" @if($project->getActiveStatus()?->getId() == $status->getId()) selected @endif> {{$status->getName()}} </option>
                                                    @endforeach
                                               @else
                                                   <option value="{{ $project->getActiveStatus()?->getId() }}" > {{$project->getActiveStatus()?->getName()}} </option>
                                               @endif
                                        </select>
                                    </div>
                                </div>
                                     ...
                                     <div class="col-md-12">
                                         ...
                                         <select name="priority" id="priority" class="form-control select2" data-placeholder="{{ __('Priority') }}">
                                            @foreach(\App\Domains\Projects\Enums\ProjectPriorityEnum::cases() as $priority)
                                                <option value="{{ $priority->value }}" @if($project->getPriority()?->value == $priority->value) selected @endif> {{ $priority->value }} </option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="col">
                                <div class="form-group row mb-1 mt-1">
                                    <div class="col-md-12">
                                        ...
                                        <select name="owner_id" class="form-control select2  @if($hasAccessToStatus ?? false) filter_owner @endif" id="filter_owner" data-placeholder="Manager">
                                            <option value="{{$project->getOwnerId()}}" selected> {{$project->getOwner()?->getName()}}  </option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <select type="text" name="assignees[]" class="form-control select2 filter_assignees"
                                            ...
                                            @foreach($project->getAssignees() as $assignee)
                                                <option value="{{$assignee->getId()}}" selected> {{$assignee?->getName()}}  </option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="col">
                                <div class="form-group row mb-1 mt-1">
                                    <div class="col-md-12">
                                        ...
                                        <input type="text" name="start_date" id="start_date" placeholder="dd/mm/yyyy" autocomplete="off" value="{{ $project->getStartDate()?->format('d/m/Y') }}" class="form-control" data-flatpickr="date">                                    </div>
                                </div>
                            </div>
                            <div class="col">
                                <div class="form-group row mb-1 mt-1">
                                    <div class="col-md-12">
                                        ...
                                        <input type="text" name="deadline" id="deadline" placeholder="dd/mm/yyyy" autocomplete="off" value="{{ $project->getDeadline()?->format('d/m/Y') }}" class="form-control" data-flatpickr="date">                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
...
@section('page-script')
    ...
    <script type="module">
        ...
        addOptions('{{ $project->getCategory()?->value }}');
        ...
        function addOptions(categoryId) {
            ...
            fetch(url)
                ...
                .then(data => {
                    ...
                    $categoryStatus.val('{{ $project->getCategoryStatus()?->value }}').trigger('change');
                })
                .catch(error => {
