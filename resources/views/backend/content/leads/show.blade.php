...
@section('title', $lead->getCompany()->getName())
...
@section('content-header-breadcrumbs')
    ...
    <li class="breadcrumb-item active">{{$lead->getCompany()->getName()}}
    </li>
@endsection
@section('content')
    <div class="tab-content p-0">
        <div class="tab-pane fade show active" id="navs-pills-top-details" role="tabpanel">
            ...
            <div class="row">
                <div class="col-lg-6">
                    <form id="form" method="POST" action="{{ route('admin.leads.update', $lead->getId()) }}" class="form-horizontal needs-validation"
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="form-group row mb-3 mt-1 align-items-center">
                                    <label for="leadCompanyId" class="col-form-label col-md-4">Εταιρεία</label>
                                    <div class="col-md-8">
                                        <select name="leadCompanyId" id="leadCompanyId" class="form-control companies-select select2" data-placeholder="{{ 'Εταιρεία' }}" data-allow-clear="true" required>
                                            @if($lead->getCompanyId())
                                                <option value="{{$lead->getCompanyId()}}" selected> {{$lead?->getCompany()?->getName()}}  </option>
                                            @endif
                                        </select>
                                    ...
                                    <div class="col-md-8">
                                        <select name="salesPersonId" id="salesPersonId" class="form-control select2 salesPersons" data-placeholder="Επιλέξτε πωλητή" data-allow-clear="true">
                                            ...
                                            @foreach($salesPersons ?? [] as $salesPerson)
                                                <option value="{{$salesPerson->getId()}}" @if($lead->getSalesPersonId() == $salesPerson->getId()) selected @endif >{{$salesPerson->getName()}}</option>
                                            @endforeach
                                        </select>
                ...
                <div class="col-lg-6">
                    @include('backend.content.companies.includes.showDetails', ['company' => $lead->getCompany()])
                </div>
            </div>
        </div>
        @foreach($company->getMorphables() ?? [] as $morph)
